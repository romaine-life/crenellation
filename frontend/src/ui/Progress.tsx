...
const REMAINING = [
  ...
  ['How far the two translations agree', 'Every write to work RAM is identical - all 300,000 recorded, under all six input patterns: attract, one, two and three players, the service switch, and a long idle run. The assertion is identity now, not a floor. Four faults in the lift did the damage, and three of them were rules the recompiler already had right and only the decompiler had dropped: bclr, bset and bchg set no flags at all, so a branch after a bit test read a stale one; absolute short addressing did not sign-extend, so $ffff.w became 0x0000FFFF where the chip makes it 0xFFFFFFFF; and the shifts set neither N nor Z. The fourth was double bookkeeping - a routine that ran past the end of its own case range was ticked twice for one instruction. The two runs also reach the same interrupt poll points with equal cycle clocks at every one, so the cost model is exact and drift explains nothing. The screens are the same picture: rebuild without the one deliberate rule change and all 80,640 pixels match, so the 380 that differ with it are that change and nothing else - the edit moves a tile index, which is why memory stays identical while those pixels do not. And the whole-game comparison now runs at each pattern full length - 2,400 frames of attract, 3,600 with one player and with two, 3,000 with all three stations, 1,800 on the service switch, 4,800 idle - in 67 seconds, where it had never finished at all. Every pattern agrees byte for byte to frame 351, and the service switch throughout. That one remaining difference is the same in all of them, and it is not a fault in the translation: it is a register spilled to the stack, holding one of the four patterns a self-test loop cycles through while it waits for the frame interrupt. The chip leaves that loop between instructions and the decompiled code leaves it at the head of a block - which is the design decision that lets the sound driver stop waiting at all - so the two come out holding different patterns. Byte-identical through a whole game and interrupts polled at block heads cannot both be had; what can be, and is, is that the difference is exactly this and nothing else.'],
