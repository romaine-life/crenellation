...
const REMAINING = [
  ...
  ['How far the two translations agree', 'Work RAM is identical for 576,837 consecutive writes, measured on 2026-08-01 by raising the recording cap - the 300,000 reported before was where the harness stopped looking, not where the two runs part, and it read as an identity claim while being a floor. Both numbers are now reported together. Five faults in the lift have been fixed, and four of them were rules the recompiler already had and only the decompiler had dropped: bclr, bset and bchg set no flags, so a branch after a bit test read a stale one; absolute short addressing did not sign-extend; the shifts set neither N nor Z; the logic group set no flags either; and a bit test on a hardware port was lifted as two reads of it where the chip reads once. The whole-game comparison runs every pattern to its own end - 2,400 frames of attract, 3,600 with one player and with two, 3,000 with all three stations, 366 on the service switch, 4,800 idle - in 81 seconds, comparing work RAM, the playfield and the palette every frame. Every pattern agrees byte for byte to frame 389, up from 351, and the service switch agrees throughout: it is the one pattern that masks interrupts to level 7 for its whole duration, so no exception frame is ever pushed. The screens are the same picture - rebuild without the one deliberate rule change and all 80,640 pixels match. What remains is one divergence, and it is not what it was thought to be. It was read as interrupt timing for two sessions; it is not. Narrowed to the single word the two runs disagree on, they agree on eight writes, part at the ninth, and then repeat that identical difference - a value six too low - unchanged across four thousand more. Drift accumulates and this does not, so it is an ordinary computation going wrong the same way every time, somewhere below the sound sequencer. Three timing fixes were built, measured and reverted before that was established.'],
