...
const REMAINING = [
  ...
  ['How far the two translations agree', 'The recompiled and decompiled games are run side by side and compared two ways. Their writes to work RAM are identical for 43,523 of them under the worst input pattern and 55,120 under attract. Their screens differ in 380 pixels of 80,640 at frame 600. What the two dispatchers do between instructions now agrees exactly: they reach the same 1,881,078 interrupt poll points, and their cycle clocks are equal at every one - so the cost model is right and drift is not the cause of anything, which took retiring three explanations that sounded better than they measured. Two faults did the damage and are fixed: a routine that ran past the end of its own case range was ticked twice for one instruction, and an address the game enters at was a block head in its own function but merged away inside the routine containing it. What is left is a real disagreement in the main line, not a seam.'],
