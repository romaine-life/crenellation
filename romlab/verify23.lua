...
local OUT = "D:/repos/crenellation/romlab/out/verify23/"
...
local function build_cases()
  ...
  while p < 0xFF90 do
    ...
    for who = 0, 2 do
      CASES[#CASES+1] = { id = pid, script = p + 1, x = 20, y = 14, who = who }
    end
...
emu.register_frame_done(function()
  ...
  if returned then
    ...
    local fh = io.open(OUT..string.format("p%02X-%d.bin", c.id, c.who), "wb"); fh:write(table.concat(t)); fh:close()
    log:write(string.format("R %02X %d %04X %d %d %08X", c.id, c.who, c.script, c.x, c.y,
                            cpu.state["D0"].value % 0x100000000)..NL)
  elseif waited < 3 then return
  else log:write(string.format("R %02X %d %04X %d %d NORETURN", c.id, c.who, c.script, c.x, c.y)..NL) end
